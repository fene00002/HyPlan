...
std::string get_cwd() 
{
    ...
    if (cwd == NULL) {
		printf("IS-DESPOT::[UTIL] Invalid current working directory: Empty.\n");
		exit(-1);
	}
...
std::string get_date()
{
    ...
    char time_string[std::size("dd.mm.yyyy_hh.mm.ss")];
    std::strftime(std::data(time_string), std::size(time_string), "%d.%m.%Y_%H.%M.%S", std::gmtime(&time));
