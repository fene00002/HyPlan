...
RandomPolicy::RandomPolicy(const DSPOMDP* model, const vector<double>& action_probs,
	...
	if (fabs(sum - 1.0) >= 1.0e-8) {
		printf("IS-DESPOT::[%s] Invalid probability distribution: Expected total probability mass of 1, got %.4f.\n",
		__PRETTY_FUNCTION__, sum);
		exit(-1);
	}
}
...
ACT_TYPE ModeStatePolicy::Action(const vector<State*>& particles,
	...
	if (mode == NULL) {
		printf("IS-DESPOT::[%s] Invalid mode particle: Empty.\n", __PRETTY_FUNCTION__);
		exit(-1);
	}
