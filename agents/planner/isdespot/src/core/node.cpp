...
VNode::VNode(vector<State*>& particles, int id, int depth, QNode* parent, OBS_TYPE edge) :
			 ...
			 edge_(edge),
			 vstar(this),
			 lstm_state(ModelParams::LSTM_STATE_SIZE, 0) {;}

VNode::~VNode() {
	for (int a = 0; a < children_.size(); a++) {
		QNode* child = children_[a];
		if (child == NULL) {
			printf("IS-DESPOT::[%s] Invalid child node: NULL.\n", __PRETTY_FUNCTION__);
			exit(-1);
		}
		delete child;
	}
