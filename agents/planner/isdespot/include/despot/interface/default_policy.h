...
struct ValuedAction {
	...
	ValuedAction(ACT_TYPE _action, double _value, double _uncertainty) : 
		action(_action), value(_value), uncertainty(_uncertainty) {;}
...
private:
	...
	ValuedAction RecursiveValue(const std::vector<State*>& particles, RandomStreams& streams, History& history) const;
...
public:
	 ...
	 */
	virtual ACT_TYPE Action(const std::vector<State*>& particles, RandomStreams& streams, History& history) const = 0;
