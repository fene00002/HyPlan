...
struct COORD
{
  ...
  static double EuclideanDistanceSimple(const COORD& lhs, const COORD& rhs);
};
...
inline double COORD::EuclideanDistanceSimple(const COORD& lhs, const COORD& rhs) {
  double result = (lhs.x - rhs.x)*(lhs.x - rhs.x) + (lhs.y - rhs.y)*(lhs.y - rhs.y);
  return result;
}
